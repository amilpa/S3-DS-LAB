...
int main() {
  ...
  matrix s_matrix_1[size1+1];
  s_matrix_1[0].value=size1;

  printf("Enter the number of rows and coloumns: ");
  scanf("%d %d", &s_matrix_1[0].row, &s_matrix_1[0].col);
  ...
  matrix s_matrix_2[size2+1];
  s_matrix_2[0].value=size2;

  printf("Enter the number of rows and coloumns: ");
  scanf("%d %d", &s_matrix_2[0].row, &s_matrix_2[0].col);

  printf("Enter the sparse matrix representation: ");
  for (int i = 1; i <= size2; i++)
    scanf("%d %d %d", &s_matrix_2[i].row, &s_matrix_2[i].col, &s_matrix_2[i].value);

  if (s_matrix_1[0].row != s_matrix_2[0].row &&
      s_matrix_1[0].col != s_matrix_2[0].col) {
  ...
  for (int i = 0; i < s_matrix_1[0].row; i++)
    for (int j = 0; j < s_matrix_1[0].col; j++) {
      if (s_matrix_1[x].row == i && s_matrix_1[x].col == j &&
          s_matrix_2[y].row == i && s_matrix_2[y].col == j) {
  ...
  for (int i = 0; i <= s_matrix_sum[0].value; i++)
    printf("%d %d %d\n", s_matrix_sum[i].row, s_matrix_sum[i].col,
           s_matrix_sum[i].value);
